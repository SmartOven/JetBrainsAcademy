...
public class XmlToJsonConverter implements Converter<XmlElement, JsonObject> {
    ...
    private JsonValue convertValue(XmlElement xml) {
        ...
        if (xml.value == null) {
            ...
            if (isArray(elements)) {
                ...
                for (XmlElement element : elements.getValues()) {
    ...
    private boolean isArray(XmlElements elements) {
        return !elements.getValues().isEmpty() && elements.getValues()
                .stream()
                .map(e -> e.name)
                .allMatch(elements.getValues().get(0).name::equals);
    }
}
